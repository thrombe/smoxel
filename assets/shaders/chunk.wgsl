...
//                 return vec4(march / 200.0, 1.0);
//             }
//             // var o = o + ray_dir * last_t;
//             // var march = floor(o) + 0.5;
//             // var marchu = vec3<u32>(march);
//             // var t = dt * (step * (0.5 - fract(o/2.0)) + 0.5);
//             // var mod4 = marchu % 4u;
//             // for (var i2 = 0u; i2 < 4u; i2 += 1u) {
//             //     if (any(mod4 >= 4u)) {
//             //         break;
//             //     }
//             //     let m = mod4 << vec3(0u, 1u, 2u);
...
//             //         masku = vec3<u32>(t.xyz <= min(t.yzx, t.zxy));
//             //         t += mask * dt;
//             //         march += mask * step;
//             //         mod4 += masku * stepu;
//             //         marchu += masku * stepu;
...
//             last_t = min(t.x, min(t.y, t.z));
//             mask = vec3<f32>(t.xyz <= min(t.yzx, t.zxy));
//             masku = vec3<i32>(t.xyz <= min(t.yzx, t.zxy));
//             t += mask * dt;
//             march += mask * step;
//             marchu += masku * stepu;
//         }
...
//     // o *= step;
//     // how much t untill we hit a plane along this axis
//     let offset_mip0 = dt * (step * (0.5 - fract(o)) + 0.5);
//     let offset_mip1 = dt * (step * (0.5 - fract(o/2.0)) + 0.5)*2.0;
//     let offset_mip2 = dt * (step * (0.5 - fract(o/4.0)) + 0.5)*4.0;
//     var offset = vec3(0.0);
//     var t = offset_mip2;

//     let sides = array(side, side/2u, side/4u, side/8u, side/16u, side/32u);
//     var voxel: u32;
//     var mip: vec2<u32>;
//     var mask: vec3<f32>;
//     var masku: vec3<u32>;
//     // let bitmasks = get_bitmasks();
//     // let bytemasks = get_bytemasks();
//     var p = 4.0;
//     var pu = 4u;
//     for (var i = 0u; i < (side / 4u) * 3u - 2u; i += 1u) {
//         if (voxel != 0u) {
...
//             // pu = 2u;
//             // march -= mask * step * p;
//             // marchu -= masku * stepu * pu;
//             t -= mask * dt * p;
//             let mt = mask * t;
//             o += ray_dir * max(mt.x, max(mt.y, mt.z));
//             march = floor(o) + 0.5;
//             marchu = vec3<u32>(march);
//             if (true) {
//                 // return vec4(o / 200.0, 1.0);
//             }
...
//                 // marchu = vec3(0u, 0u, 0u);
//                 var m1 = (marchu%4u) > 1u;
//                 var m2 = vec3<u32>(m1) << vec3<u32>(0u, 1u, 2u);
//                 var m3 = m2.x | m2.y | m2.z;
//                 var present = getMipByte(mip, m3);

//                 // if (marchu + mask * stepu) {
//                 //     break;
//                 // }
                
//                 if (present != 0u) {
//             if (true) {
//                 return vec4<f32>(march/200.0, 1.0);
//             }
//                     for (var i3 = 0u; i3 < 4u; i3+=1u) {
//                         m1 = (marchu%2u) > 0u;
//                         m2 = vec3<u32>(m1) << vec3(0u, 1u, 2u);
//                         m3 = m2.x | m2.y | m2.z;
//                         present = getMipBit(present, m3);
//                         if (present != 0u) {
//                             voxel = get_voxel(march);
//                             break;
//                         } else {
